...
- **Arm a stop on every entry.** Pass `stop_price` to `create_position`. That also arms the exit ladder — a third off at +1R, stop to break-even at +1.5R, trailing from +2R — which a watcher executes on 1-minute bars between your cycles. You choose the levels; it catches the moments you would otherwise sleep through. A position opened without a stop has no ladder and nothing will close it until you next run.
