      ...
      <div v-else class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-4">
          <template v-for="(crypto, index) in displayCryptoPrices" :key="crypto.symbol">
            ...
            <div v-if="index < displayCryptoPrices.length - 1" class="h-4 w-px bg-mono-divider"></div>
          </template>
        </div>
        ...
        <div class="flex items-center space-x-6">
          <div class="flex items-center space-x-3">
            ...
            <span :class="[
              'text-xs font-bold',
              highest.change >= 0 ? 'text-mono-up' : 'text-mono-down'
            ]">
              {{ formatChange(highest.change) }}
            </span>
          </div>
          
          <!-- <div class="h-4 w-px bg-mono-divider"></div>
          ...
          <div class="flex items-center space-x-2">
            ...
            <span :class="[
              'text-xs font-bold',
              lowest.change >= 0 ? 'text-mono-up' : 'text-mono-down'
            ]">
